...
def compute_distances(Xtrain, X):
    ...
    dists = np.sum(Xtrain ** 2, axis = 1).reshape(Xtrain.shape[0], 1).T + np.sum(X ** 2, axis = 1).\
        reshape(X.shape[0], 1) - 2 * np.dot(X, Xtrain.T)
    return dists
...
def predict_labels(k, ytrain, dists):
    ...
    # Method 1
    # import collections
    # ypred = np.zeros(shape=(len(dists), 1), dtype = int)
    # for i in range(len(dists)):
    #     pred = list()
    #     for j in range(k):
    #         min_ = np.argmin(dists[i])
    #         dists[i][min_] = 9999
    #         pred.append(ytrain[min_])
    #     pred = sorted(pred)
    #     pred = collections.Counter(pred)
    #     ypred[i][0] = int(pred.most_common(1)[-1][0])
    # return ypred

    # Method 2
    ypred = np.zeros(shape=(len(dists), 1), dtype=int)
    sorted_indexs = np.argsort(dists, axis = 1)
    sorted_indexs = sorted_indexs[:,:k]
    for i in range(len(sorted_indexs)):
        for j in range(len(sorted_indexs[i])):
            sorted_indexs[i][j] = ytrain[sorted_indexs[i][j]]
        # sorted_indexs[i] = sorted(sorted_indexs[i])
        counts = np.bincount(sorted_indexs[i])
...
def compute_accuracy(y, ypred):
    ...
    #				 YOUR CODE HERE					                    #
    #####################################################
    def calculate(i, y, ypred):
        if i == len(y):
            return 0
        else:
            if y[i] == ypred[i]:
                return 1 + calculate(i+1, y, ypred)
            return 0 + calculate(i+1, y, ypred)
    i = 0
    acc = calculate(i, y, ypred) / len(y)
    return acc
...
def find_best_k(K, ytrain, dists, yval):
    ...
    max_accuracy_till_now = -9999
    for i in K:
        ypred = predict_labels(i, ytrain, dists)
        acc = compute_accuracy(ypred, yval)
        validation_accuracy.append(acc)
        if acc > max_accuracy_till_now:
            max_accuracy_till_now = acc
            best_k = i
