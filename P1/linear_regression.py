...
def regularized_linear_regression(X, y, lambd):
    ...
    #				 YOUR CODE HERE					                    #
    #####################################################
    t = np.matmul(np.transpose(X), X)
    dim = len(t)
    w = np.matmul(np.matmul(np.linalg.inv(np.add(t, np.identity(dim) * lambd)), np.transpose(X)), y)
    return w
...
def test_error(w, X, y):
    ...
    # Method 2
    # error = ((np.dot(X, w.reshape(w.shape[0], 1)) - y.reshape(y.shape[0], 1)) ** 2) / y.shape([0])

    return np.sum((np.dot(X, w.reshape(w.shape[0], 1)) - y.reshape(y.shape[0], 1)) ** 2, axis = 0)[0] / y.shape[0]
