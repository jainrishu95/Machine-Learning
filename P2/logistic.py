...
def binary_train(X, y, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    for i in range(max_iterations):
        delta = sigmoid(np.dot(X, w) + b) - y
        b -= step_size * (np.sum(delta, axis=0) / N)
        w -= step_size * (np.dot(X.T, delta) / N)
...
def multinomial_train(X, y, C, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    eye = np.eye(C)[y].T
    for i in range(max_iterations):
        probability = np.dot(w, X.T) + b[:, np.newaxis]
        probability = np.exp(probability - np.argmax(probability, axis=0))
        sum_ = np.sum(probability, axis=0)
        probability = (probability / sum_) - eye
        w -= step_size * (np.dot(probability, X))/N
        b = b - (step_size * np.sum(probability, axis=1)/N)
...
def multinomial_predict(X, w, b):
    ...
    """
    probability = np.dot(w, X.T) + b[:, np.newaxis]
    preds = np.argmax(probability, axis=0)
