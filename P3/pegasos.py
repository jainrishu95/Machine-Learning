...
def objective_function(X, y, w, lamb):
    ...
    minus = 1 - np.multiply(y, np.dot(X, w))
    max_ = np.sum(np.maximum(0, minus))
    l2_norm = np.square(np.linalg.norm(w, 2))
    value1 = ((lamb / 2) * l2_norm)
    value2 = np.divide(max_, X.shape[0])
    obj_value = value1 + value2
    return obj_value
...
def pegasos_train(Xtrain, ytrain, w, lamb, k, max_iterations):
    ...
    for iter in range(1, max_iterations + 1):
        ...
        # you need to fill in your solution here
        X_train_subset = np.take(Xtrain, A_t, axis=0)
        y_train_subset = np.take(ytrain, A_t, axis=0)
        y_train_subset = y_train_subset.reshape(y_train_subset.shape[0], 1)
        A_t_plus = np.multiply(y_train_subset, np.dot(X_train_subset, w))
        A_t_plus = np.where(A_t_plus < 1)
        X_train_subset_A_t_plus = np.take(X_train_subset, A_t_plus[0], axis=0)
        y_train_subset_A_t_plus = np.take(y_train_subset, A_t_plus[0], axis=0)
        y_train_subset_A_t_plus = y_train_subset_A_t_plus.reshape(y_train_subset_A_t_plus.shape[0], 1)
        n_t = 1 / (lamb * iter)
        xy = np.sum(np.multiply(X_train_subset_A_t_plus, y_train_subset_A_t_plus), axis=0)
        xy = xy.reshape(xy.shape[0], 1)
        half1 = ((1 - (n_t * lamb)) * w)
        half2 = ((n_t / k) * xy)
        w_t_half = half1 + half2
        numerator = 1 / (lamb ** 0.5)
        l2_norm = np.linalg.norm(w_t_half, 2)
        w = np.multiply(np.minimum(1, numerator / l2_norm), w_t_half)
...
def pegasos_test(Xtest, ytest, w_l):
    ...
    test_acc = np.sum(np.equal(ytest, np.sign(np.dot(Xtest, w_l)))) / Xtest.shape[0]
    return test_acc
...
def pegasos_mnist():
    ...
    for lamb in (0.01, 0.1, 1):
        ...
        #objective_function(Xtrain, ytrain, w, lamb)
