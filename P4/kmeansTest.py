...
def transform_image(image, code_vectors):
    ...
    for i in range(image.shape[0]):
        A = np.reshape(np.sum(image[i] ** 2, axis=1), (image[i].shape[0], 1))
        B = np.reshape(np.sum(code_vectors ** 2, axis=1), (code_vectors.shape[0], 1))
        AB = np.dot(image[i], code_vectors.T)
        Matrix = A + B.T - 2 * AB
        indexes = np.argmin(Matrix, axis=1)
        new_im[i] = code_vectors[indexes]
